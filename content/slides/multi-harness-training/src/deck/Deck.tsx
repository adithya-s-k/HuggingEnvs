...
import { MONO, STAGE_W, STAGE_H } from "../themes";
...
/**
 * Uniformly scale the fixed canvas to fit whatever space the stage container
 * has. Measuring the container (not the window) means the slide re-scales as
 * the drawer pushes it narrower.
 */
...
export function Deck({ slides }: { slides: Slide[] }) {
  ...
  // Control surface for scripts/export-deck.mjs — it drives the deck by index
  // and screenshots [data-stage]. Harmless during a talk.
  useEffect(() => {
  ...
  if (printing) return <PrintMode slides={slides} onDone={() => setPrinting(false)} />;
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: T.bg, display: "flex", overflow: "hidden" }}
    >
      {/* Drawer is a real layout child, so it pushes the stage instead of covering it. */}
      <motion.div
      ...
      >
        ...
        >
          ...
          <AnimatePresence mode="wait" custom={dir}>
            ...
            >
              <SectionNumberContext.Provider value={slides[index].section ?? null}>
...
function ProgressBar({ index, total }: { index: number; total: number }) {
  ...
  return (
    <div
      data-chrome
      style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, zIndex: 50 }}
    >
      <motion.div
        animate={{ width: `${pct}%` }}
        transition={{ type: "spring", damping: 26, stiffness: 200 }}
        style={{ height: "100%", background: T.accent, boxShadow: glow.accent }}
      />
    </div>
  );
}
