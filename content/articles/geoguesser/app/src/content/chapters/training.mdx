...
Before you start, send every run to **one** dashboard. We use [Trackio](https://huggingface.co/spaces/HuggingEnvs/geoguesser-trackio), which is a Space plus a bucket, so the runs outlive the Jobs that produced them and sit on a shared axis where they can actually be compared. Reading three runs in three tabs is how you end up believing things that are not true. The whole dashboard is embedded at the end of this article, and every training figure below is built from its database.
