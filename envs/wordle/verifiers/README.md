...
`WordleToolkit` (in `env.py`) wraps the shared `WordleGame` from `envs/wordle/game.py` and exposes two methods as tools:
...
The `WordleGame` itself lives at `envs/wordle/game.py` (sibling), shared across all six framework folders.
