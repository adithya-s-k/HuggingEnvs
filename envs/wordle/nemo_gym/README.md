...
> ⚠️ Same caveat as the Jupyter NeMo Gym env: **local server requires Ray**, which fails on shared HF / SLURM cluster nodes (`gcs_server` can't bind). The deployed Space is the recommended path. See `envs/jupyter_agent/nemo_gym/README.md` for the full story.
