"""End-to-end smoke test for a deployed Gemma-4-E4B vLLM server.

Tests every mode documented in the Gemma 4 model card + vLLM recipe:
  1. Raw HTTP /v1/models and /v1/chat/completions
  2. OpenAI SDK — text-only, instruct (no thinking) mode
  3. OpenAI SDK — text-only, thinking mode (reasoning chain)
...
Usage:
    python test_gemma4_vllm.py --base-url http://ip-26-0-xxx-xxx:8000/v1
    python test_gemma4_vllm.py --base-url ... --skip image
"""
...
# Image from the official vLLM Gemma4 recipe example.
DEMO_IMAGE_URL = (
    "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3a/Cat03.jpg/1200px-Cat03.jpg"
)

# Sampling parameters from the model card "Best Practices".
GEMMA_SAMPLING = {
    "temperature": 1.0,
    "top_p": 0.95,
    "extra_body": {"top_k": 64},
...
# -----------------------------------------------------------------------------
# Test 2 — text, no thinking
# -----------------------------------------------------------------------------
def test_text_instruct(client: OpenAI) -> bool:
    hr("TEST 2 — OpenAI SDK · text · no thinking")
    try:
        t0 = time.time()
        ...
        rc = getattr(msg, "reasoning", None) or getattr(msg, "reasoning_content", None)
        print(f"reasoning (should be empty): {rc!r}")
...
# -----------------------------------------------------------------------------
# Test 3 — text, thinking enabled
# -----------------------------------------------------------------------------
def test_text_thinking(client: OpenAI) -> bool:
    hr("TEST 3 — OpenAI SDK · text · thinking enabled")
    try:
        t0 = time.time()
        resp = client.chat.completions.create(
            model=MODEL,
            messages=[
                {
                    "role": "user",
                    "content": "A snail climbs 3 ft each day in a 20-ft well, slips back 2 ft each night. How many days to escape?",
                },
            ],
            max_tokens=4096,
            **GEMMA_THINKING,
        ...
        elapsed = time.time() - t0
        msg = resp.choices[0].message
        # Gemma4 reasoning parser exposes via `reasoning` field per the vLLM recipe.
        rc = getattr(msg, "reasoning", None) or getattr(msg, "reasoning_content", None)
        print(f"({elapsed:.1f}s)")
        print(f"reasoning: {truncate(rc, 400) if rc else '(none — model may have skipped <|channel> block)'}")
...
def main() -> int:
    ...
    parser.add_argument("--base-url", required=True, help="vLLM URL, e.g. http://ip-26-0-xxx-xxx:8000/v1")
