"""
Jupyter Agent Environment Client.

Connects to a running jupyter-agent-openenv server and exposes
notebook tools via the MCP tool-calling interface.
...
Example with Docker:
    >>> env = JupyterAgentEnv.from_docker_image("jupyter-agent-openenv:latest")
    >>> try:
    ...     env.reset()
...
class JupyterAgentEnv(MCPToolClient):
    """
    Client for the Jupyter Agent environment.
