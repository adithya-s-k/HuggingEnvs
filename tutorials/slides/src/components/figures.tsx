// Small inline logos, ported from hf-motion figures.
...
// RaisedHand — a clean geometric "high-five" hand, matching the mono aesthetic.
export function RaisedHand({
  ...
  return (
    <svg width={size} height={(size * 60) / 52} viewBox="0 0 52 60" fill={color} aria-label="Raised hand">
      {/* palm */}
      <rect x={8} y={33} width={38} height={24} rx={12} />
      {/* fingers */}
      <rect x={11} y={15} width={7} height={28} rx={3.5} />
      <rect x={20} y={9} width={7} height={34} rx={3.5} />
      <rect x={29} y={13} width={7} height={30} rx={3.5} />
      <rect x={38} y={19} width={7} height={24} rx={3.5} />
      {/* thumb */}
      <rect x={1} y={36} width={7} height={22} rx={3.5} transform="rotate(-40 4.5 47)" />
    </svg>
  );
}

// HFMark — official Hugging Face logo (the smiley face).
// SVG paths sourced from huggingface.co/front/assets/huggingface_logo-noborder.svg
export function HFMark({ size = 22 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={(size * 88) / 95}
