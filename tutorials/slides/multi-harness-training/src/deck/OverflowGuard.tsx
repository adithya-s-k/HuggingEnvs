...
/**
 * DEV-ONLY. After a slide's animations settle, measure every descendant against
 * the 1280×720 canvas and flag anything spilling past the edge — which the
 * stage's overflow:hidden would otherwise clip silently. Catches the single most
 * common authoring mistake: too much content on one slide.
 */
export function OverflowGuard({ id, children }: { id: string; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(check, 1700); // let entrance animations settle
