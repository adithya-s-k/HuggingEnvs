...
Run:
    ...
    curl -sSL <raw>/tools/jupyter_launch.py | python3 -        # one-liner (FLAVOR=t4-small to pick GPU)
    irm  <raw>/tools/jupyter_launch.py | python -             # Windows PowerShell
...
DEFAULT_REPO = "https://github.com/adithya-s-k/HuggingEnvs.git"
...
def main():
    ...
    bootstrap = (
        ...
        f"mkdir -p {MOUNT}; for d in /repo/[0-9][0-9]-*/notebooks; do cp -rn \"$d\"/. {MOUNT}/ 2>/dev/null || true; done; "
        f"cd {MOUNT}; "
